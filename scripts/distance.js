...
function getDistanceFromLatLonInKm(lat1, lon1, lat2, lon2) {
  ...
  function newPointAdded(newPoint) {
    // expecting an object form of the new row
    ...
    else if (globalDataQueue.length < 2){
        console.log("Not enough points yet")
        return false;
    }
    ...
    if (lastPoint.Altitude < newPoint.Altitude) {
      //if we still rising, we don't really care to make prediction
      console.log("Still rising!");
      return false;
    }
    yVelocity = (newPoint.Altitude - lastPoint.Altitude) / ((newPoint.created_at - lastPoint.created_at)/1000); //convert to seconds
    let timeRemaining = timeToHitGround(yVelocity, newPoint.Altitude);
    console.log("Time to hit the ground (s):", timeRemaining);
  
    //get x velocity
    let temp = getDistanceFromLatLonInKm(
        lastPoint.Latitude,
        lastPoint.Longitude,
      newPoint.Latitude,
      newPoint.Longitude
    ...
    let speed =
      distanceBetweenPoints / ((newPoint.created_at - lastPoint.created_at)/1000);
